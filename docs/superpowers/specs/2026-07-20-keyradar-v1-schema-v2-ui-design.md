...
现有应用和 Windows 系统规则全部迁移为 Schema v2 JSON。一个应用可以包含
多个变体文件，首轮正式规则数量以实际完成迁移、取得证据并通过审核的内容
为准。应用数量和 JSON 文件数量都不得写死在程序、构建脚本、测试或文档中。
Windows 系统规则单独管理，但不参与任何固定数量判断。

规则包构建只验证内容非空、身份唯一、Schema 合法、来源与隐私要求、安全
边界和签名，不以某个预设应用总数作为成功条件。规则库页面根据实际加载
结果动态显示应用数、变体数和热键数。
